...
var Output = React.createClass({
  ...
  render: function () {
    ...
    let kernelName = getKernelName(node.itreed, store.globals.itreed.kernels)
    var className = cx({
      'm_IPython': true,
    ...
    return <div className={className} ref='ipython_view' onClick={this.props.setActive}>
      ...
      {showTimes(node, this.props.kernelSession, kernelName, !store.globals.itreed.multiKernels)}
    </div>
  },
})

function getKernelName(config, kernels) {
  const key = config.server + ':' + config.kernel
  const kernel = kernels[key]
  if (!kernel) return false
  if (!kernel.variants) return `Kernel ${key} failed to initialize`
  const variant = kernel.variants[config.variant || 'default']
  if (!variant) return false
  if (!config.variant || config.variant === 'default') {
    return kernel.title
  }
  return kernel.title + ' (' + (variant.title || config.variant) + ')'
}
