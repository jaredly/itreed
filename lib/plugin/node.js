...
function showOutput(output, i) {
  ...
  if (output['text/latex']) {
    return <div className='m_IPython_output' key={i}
      dangerouslySetInnerHTML={{__html: output['text/latex']}}/>
  }
  */
  if (output['image/png']) {
...
module.exports = {
  classes: function (node, state) {
    if (!state.kernelSession && node.type === 'ipython') {
      return 'list_item-ipython-disabled'
    }
  },
  blocks: {
    belowbody: function (node, actions, state) {
      ...
      return <div className={className} ref='ipython_view' onClick={
        () => actions.toggleDisplayCollapse(node.id)
      }>
