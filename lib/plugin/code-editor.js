  ...
  componentDidMount: function () {
    var betterTab = function (cm) {
      ...
      if (cursor.ch > 0 && line[cursor.ch - 1] !== ' ') {
        ...
        var cb = (cm, callback, options) => {
          onComplete(line, cursor.ch, (results, matched) => {
            callback({
              list: results,
              from: {line: cursor.line, ch: cursor.ch - matched.length},
              to: pos,
            })
          })
        }
        ...
        // this.props.onComplete(line, cursor.ch, 
      }
